...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
	if (record->event.pressed) {
		switch (keycode) {
			case TYPEDON:
				SEND_STRING("[typed on an eggplant]");
				return false;
            case BRACKETBRACKET:
                if (record->event.pressed) {
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	[FUNKY] = LAYOUT(
		KC_VOLU, BRACKETBRACKET,_______,KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, _______,
		KC_VOLD, CURLYCURLY, vvv,   KC_F7, KC_F8, KC_F9, KC_F10, KC_F11, KC_F12, _______,
